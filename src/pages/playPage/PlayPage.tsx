  ...
  return (
    <>
      {'Finished' in gameState ? (
        <EndGameDialog winnerId={gameState.Finished.winner} enemyPlayer={enemyPlayer} />
      ) : null}
      {'Playing' in gameState ? (
        <>
          <EnemyGamePart
            setTarget={setTargetCard}
            cardList={enemyCardList}
            user={enemyPlayer}
            turn={gameState.Playing.state.turn}
          />
          <ActionButtons
            turn={gameState.Playing.state.turn}
            gameId={gameState.Playing.state.id}
            attackCard={attackCard}
            targetCard={targetCard}
          />
          <AllyGamePart
            setAttack={setAttackCard}
            cardList={myCardList}
            user={myPlayer}
            turn={gameState.Playing.state.turn}
          />
            ...
            >
              {gameState.Waiting.players.map((player) => (
                <Chip
                  ...
                  icon={player.ready ? <Check /> : <LoaderIcon />}
                />
              ))}
            </Box>
          </Box>
